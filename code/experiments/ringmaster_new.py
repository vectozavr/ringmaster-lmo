...
def greed_search(gammas, max_delays, time_lim, optimizer_name):
    ...
    for gamma in gammas:
        for max_delay in max_delays:
            ...
            run_label = rf"$\gamma={gamma}$"
            if optimizer_name == "RingmasterASGD":
                run_label += rf", $R={max_delay}$"
            elif optimizer_name == "RennalaSGD":
                run_label += rf", $B={max_delay}$"
    ...
    plt.title(f"{optimizer_name} greedy search")
...
gamma = 0.04
optimizer = Momentum_Normalized_RingmasterASGD(transport, point, parameter_agnostic=False, gamma=gamma, alpha=0.5, threshold=6)
...
plt.semilogy(iteration_times, iteration_grads, label=rf"Norm: $\gamma={gamma}$", linestyle='solid', marker=markers[4], markersize=14, markevery=max(1, len(iteration_times) // 10), color=colors[4])
...
gamma = 2
optimizer = Momentum_Normalized_RingmasterASGD(transport, point, parameter_agnostic=True, gamma=gamma)
print(optimizer.__class__.__name__)
...
plt.semilogy(iteration_times, iteration_grads, label=rf"PA Norm: $\gamma={gamma}$", linestyle='solid', marker=markers[3], markersize=14, markevery=max(1, len(iteration_times) // 10), color=colors[3])
...
optimizer = RingmasterASGD(transport, point, max_delay=6, gamma=0.03125)
print(optimizer.__class__.__name__)
...
plt.semilogy(iteration_times, iteration_grads, label=r"Ringmaster ASGD: $\gamma=0.2$, $R=6$", linestyle='solid', marker=markers[0], markersize=14, markevery=max(1, len(iteration_times) // 10), color=colors[0])
...
optimizer = RennalaSGD(transport, point, gamma=0.2, batch_size=6) # tuned parameters
print(optimizer.__class__.__name__)
